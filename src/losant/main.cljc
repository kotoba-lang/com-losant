(ns losant.main
  "Kotodama WASM entrypoint for the Losant clean-room actor (L4) — Clojure port.
  ...
  facts (`losant.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "losant")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"losant.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "losant-compat" :tier tier :entities entities} 200])
...
(defn start! [] :losant-compat/ready)
